...
from aws_cdk import aws_iam, aws_secretsmanager
...
class SharedInfraStack(cdk.Stack):
    def __init__(self, scope: cdk.Construct, id: str, **kwargs) -> None:
        ...
        #Deploying config/settings files
        aws_s3_deployment.BucketDeployment(
            self,
            'configAssets',
            sources=[aws_s3_deployment.Source.asset('../src/layers/config_files.zip')],
            destination_bucket=s3_config_bucket
        )
        
        #Deploying glue scripts files
        aws_s3_deployment.BucketDeployment(
            self,
            'glueJobsAssets',
            sources=[aws_s3_deployment.Source.asset('../src/glue-jobs')],
            destination_bucket=s3_config_bucket,
            destination_key_prefix='glue-scripts'
        )
        
        #Deploying dependency jar files
        aws_s3_deployment.BucketDeployment(
            self,
            'glueJarsAssets',
            sources=[aws_s3_deployment.Source.asset('../src/layers/extra_jars.zip')],
            destination_bucket=s3_config_bucket,
            destination_key_prefix='libs'
        )
        

        # # Secrets manager for Salesforce 
        # #dataBucketPath = self.node.try_get_context("S3_BUCKET_NAME") + '/cl/SalesForce/'
        # salesforce_secrets_manager = aws_secretsmanager.CfnSecret(
        #     self, 'SalesforceSecrets',
        #     name="{OWNER}-{PRODUCT}-{PACKAGE}-{RESOURCENAME}-{STAGE}".format(OWNER=OWNER,PRODUCT=PRODUCT,PACKAGE=PACKAGE,RESOURCENAME="Credentials-Salesforce",STAGE=STAGE),
        #     description='CrownBio Salesforce Credentials',
        #     secret_string='{"db_username":"", \
        #                     "db_password":"", \
        #                     "db_securitytoken":"", \
        #                     "data_bucket":"'+ globalsettings.S3_DATA_BUCKET_NAME + '", \
        #                     "config_bucket":"'+ globalsettings.S3_CONFIG_BUCKET_NAME + '"}'
            
        # )
        
        # # Secrets manager for Redshift
        # redshift_secrets_manager = aws_secretsmanager.CfnSecret(
        #     self, 'RedshiftSecrets',
        #     name="{OWNER}-{PRODUCT}-{PACKAGE}-{RESOURCENAME}-{STAGE}".format(OWNER=OWNER,PRODUCT=PRODUCT,PACKAGE=PACKAGE,RESOURCENAME="Credentials-Redshift",STAGE=STAGE),
        #     description='CrownBio redshift credentials',
        #     secret_string='{"RS_HOST":"", \
        #                     "RS_PORT":"", \
        #                     "RS_USER":"", \
        #                     "RS_PASSWORD":"", \
        #                     "RS_DATABASE":"", \
