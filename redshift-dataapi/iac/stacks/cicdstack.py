...
class CICDStack(cdk.Stack):
    ...
    existing_cicd_bucket = s3.Bucket.from_bucket_name(self, "ExistingCICDBucket",
        ...
        pipeline = pipelines.CdkPipeline(
            ...
            source_action=aws_codepipeline_actions.GitHubSourceAction(
                ...
                oauth_token=cdk.SecretValue.secrets_manager(
                    "SteelHeadHQ-AWSAccountsHQ-Github-Token"
                ),
            ...
            synth_action=pipelines.SimpleSynthAction(
                project_name="{OWNER}-{PRODUCT}-{RESOURCENAME}-{STAGE}".format(
                    OWNER=OWNER,
                    PRODUCT=PRODUCT,
                    RESOURCENAME="CICDPieline-Synth",
                    STAGE=STAGE,
                ),
                source_artifact=source_artifact,
                cloud_assembly_artifact=cloud_assembly_artifact,
                subdirectory="foundationhq/iac/",
        ...
        """
    shellscript_actions_stage = pipeline.add_stage(
        stage_name = "ShellScriptAction",
        
        )

    shellscript_actions_stage.add_actions(pipelines.ShellScriptAction(
      action_name='ShellScriptActions',
      run_order=shellscript_actions_stage.next_sequential_run_order(),
      additional_artifacts=[source_artifact],
      commands=[
        'cd LifeSciencesIQ/foundationhq/iac/',
        'pip install -r requirements.txt'
      ],
